...
execute "Setting up offlinewls12indexing" do
cwd ENV['daas_ops_home'] + "/daas-ops/highlevel"
user 'oracle'
group 'oinstall'
command "export daas_node_instance=#{node[:daas_b2bx][:autowork]}/app/ &&  python offlinewls12indexing.py setup >> #{node[:daas_b2bx][:autowork]}/log/offlinewls12indexing.log 2>&1"
timeout 14400
not_if { ::File.exists?("#{node[:daas_b2bx][:autowork]}/log/offlinewls12indexing.log") }
end
